<h1 align="center">Object-Oriented UI Library</h1>
<p align="center">
...
__Библиотека была создана случайно. Изначально я создавал компоненты в своих пет-проектах, потом замемтил что компоненты переходят из проекта в проект, начал их собирать в одном месте и как итог это вылелось в мою микро "библеотечку". На просторах интернета подобных библиотек очень много, так что это лишь моё виденье компоновки "отображения" (от проекта к проекту реализация практически не меняется). Лучше всего библиотека показывает себя в архитектурных паттернах MV группы.__
...
- lib/ooui.ts — главный файл библиотеки
